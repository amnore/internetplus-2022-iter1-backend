...
public class DataFieldHelper {
    ...
    private static final String[] mainIllegalFact = new String[] { "主要违法违规事实(案由)", "主要违法违规事实（案由）", "违法事实", "违法行为类型" };
    private static final String[] punishmentBasis = new String[] { "行政处罚依据", "处罚依据" };
    private static final String[] punishmentDecision = new String[] { "行政处罚决定", "行政处罚内容", "处罚内容", "处罚类型" };
     ...
     */
    public static BankPunishment setRegardingValue(String fieldName, String value, BankPunishment dataEntity) {
        if (value != null) {
     ...
     * 检查是否所有字段都为空 (日期不做判断）
     */
    public static boolean isAllFieldNull(BankPunishment dataEntity) {
